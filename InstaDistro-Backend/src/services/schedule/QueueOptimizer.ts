...
export class QueueOptimizer {
   ...
   */
  async getOptimalTimes(accountId: string, count: number = 3): Promise<OptimalTime[]> {
    ...
    try {
      ...
      const optimalTimes: OptimalTime[] = result.rows.slice(0, count).map(row => {
  ...
  async optimizeQueue(
    ...
    const accountOptimalTimes: Map<string, OptimalTime[]> = new Map();
   ...
   */
  async getBestDaysOfWeek(accountId: string): Promise<number[]> {
    const client = await pool.connect();
   ...
   */
  async getRecommendedPostsPerDay(accountId: string): Promise<number> {
    const client = await pool.connect();

    try {
      // Get account health score
      const healthQuery = await client.query(
        'SELECT overall_score FROM account_health_scores WHERE account_id = $1',
        [accountId]
      );
      ...
      const accountQuery = await client.query(
        'SELECT account_state FROM accounts WHERE id = $1',
        [accountId]
      );
   ...
   */
  async createOptimizationReport(accountId: string): Promise<QueueOptimization> {
   ...
   */
  async balanceLoadAcrossAccounts(
    accountIds: string[],
    totalPosts: number,
    config: {
      startDate: Date;
    ...
    for (const rec of recommendations) {
      ...
      const allocated = Math.min(
        Math.floor(totalPosts * proportion),
        remaining
      );
...
export default new QueueOptimizer();
