...
import {
  WarmupTask,
  CreateWarmupTaskInput,
  UpdateWarmupTaskInput,
  WARMUP_PROTOCOL,
  getWarmupProtocolForDay,
  calculateWarmupProgress,
  isWarmupComplete,
  TaskType,
  TaskStatus,
} from '../../models/WarmupTask';

/**
 * Warmup Automation Service
 *
 * Manages the 14-day warmup protocol for new Instagram accounts
 * Generates tasks, tracks progress, and transitions accounts to ACTIVE state
 */
...
export class WarmupAutomationService {
  /**
   * Start warmup protocol for an account
   * Generates all 14 days of tasks with scheduled times
   */
  async startWarmup(accountId: string): Promise<StartWarmupResult> {
    try {
      logger.info(`Starting warmup protocol for account ${accountId}`);

      // Check if account exists
      const accountResult = await pool.query(
        'SELECT id, username, account_state FROM accounts WHERE id = $1',
        [accountId]
      );
      ...
      const existingTasks = await pool.query(
        'SELECT COUNT(*) FROM warmup_tasks WHERE account_id = $1',
        [accountId]
      );
      ...
      await pool.query(
        'UPDATE accounts SET account_state = $1, updated_at = NOW() WHERE id = $2',
        ['WARMING_UP', accountId]
      );

      // Generate tasks for all 14 days
      const tasksGenerated = await this.generateWarmupTasks(accountId);

      logger.info(`Generated ${tasksGenerated} warmup tasks for account ${account.username}`);
   ...
   */
  private async generateWarmupTasks(accountId: string): Promise<number> {
    let tasksGenerated = 0;
    const now = new Date();

    for (const dayProtocol of WARMUP_PROTOCOL) {
      ...
      for (const task of dayProtocol.tasks) {
        ...
        const taskInput: CreateWarmupTaskInput = {
   ...
   */
  async createTask(input: CreateWarmupTaskInput): Promise<WarmupTask> {
    const result = await pool.query(
      `INSERT INTO warmup_tasks (account_id, day, task_type, target_count, scheduled_time)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`,
      [input.account_id, input.day, input.task_type, input.target_count, input.scheduled_time]
    );
   ...
   */
  async getProgress(accountId: string): Promise<WarmupProgress | null> {
    try {
      // Get all tasks for this account
      const tasksResult = await pool.query(
        `SELECT * FROM warmup_tasks
         WHERE account_id = $1
         ORDER BY day, scheduled_time`,
        [accountId]
      );
      ...
      const completedDays = new Set(
        allTasks.filter(t => t.status === 'completed').map(t => t.day)
      );
      ...
      const pendingTasks = allTasks.filter(
        t => t.status === 'pending' && new Date(t.scheduled_time) > new Date()
      );
      ...
      return {
        ...
        currentDay,
        totalDays: 14,
        progressPercentage: calculateWarmupProgress(currentDay),
        tasksCompleted,
        tasksTotal,
        tasksToday,
        nextTaskTime: nextTask?.scheduled_time,
        isComplete: isWarmupComplete(currentDay) && tasksCompleted === tasksTotal,
      };
    } catch (error: any) {
   ...
   */
  async getDueTasks(limit: number = 100): Promise<WarmupTask[]> {
    try {
      const result = await pool.query(
        `SELECT * FROM warmup_tasks
   ...
   */
  async getTasksForDay(accountId: string, day: number): Promise<WarmupTask[]> {
    try {
      const result = await pool.query(
        `SELECT * FROM warmup_tasks
      ...
      logger.error('Failed to get tasks for day:', error);
      return [];
    }
  }
   ...
   */
  async updateTask(taskId: string, updates: UpdateWarmupTaskInput): Promise<WarmupTask | null> {
    try {
      const fields: string[] = [];
      const values: any[] = [];
      let paramCount = 1;
   ...
   */
  async completeTask(taskId: string, completedCount: number): Promise<boolean> {
    try {
      const updates: UpdateWarmupTaskInput = {
        status: 'completed',
   ...
   */
  private async checkWarmupCompletion(accountId: string): Promise<void> {
    try {
      ...
      if (progress.isComplete && progress.currentDay >= 14) {
        ...
        logger.info(`Account ${accountId} completed warmup! Transitioning to ACTIVE state.`);
      }
    } catch (error: any) {
      logger.error('Failed to check warmup completion:', error);
    }
  }
   ...
   */
  async pauseWarmup(accountId: string): Promise<boolean> {
    try {
      ...
      logger.info(`Warmup paused for account ${accountId}`);
      return true;
    } catch (error: any) {
   ...
   */
  async resumeWarmup(accountId: string): Promise<boolean> {
    try {
      ...
      logger.info(`Warmup resumed for account ${accountId}`);
      return true;
    } catch (error: any) {
   ...
   */
  async skipToActive(accountId: string): Promise<StartWarmupResult> {
    try {
      logger.warn(`⚠️ SKIPPING WARMUP for account ${accountId} - This is risky!`);
      ...
      logger.error('Failed to skip warmup:', error);
      return {
        success: false,
   ...
   */
  async getAccountsInWarmup(userId?: string): Promise<any[]> {
    try {
      let query = `
        SELECT a.*,
      ...
      logger.error('Failed to get accounts in warmup:', error);
      return [];
    }
  }
   ...
   */
  async getWarmupStats(userId?: string): Promise<any> {
    try {
      let query = `
        SELECT
   ...
   */
  private mapTask(row: any): WarmupTask {
    return {
      id: row.id,
...
export const warmupAutomationService = new WarmupAutomationService();
