...
describe('HealthController Integration Tests', () => {
    ...
    describe('GET /api/health/account/:id', () => {
        it('should return account health report', async () => {
            ...
            HealthMonitor_1.default.monitorAccount.mockResolvedValue(mockReport);
            ...
            expect(HealthMonitor_1.default.monitorAccount).toHaveBeenCalledWith('acc123');
        });
        it('should handle errors gracefully', async () => {
            HealthMonitor_1.default.monitorAccount.mockRejectedValue(new Error('Account not found'));
            const response = await (0, supertest_1.default)(app)
    ...
    describe('POST /api/health/account/:id/monitor', () => {
        it('should queue account monitoring job', async () => {
            ...
            HealthMonitorJob_1.queueAccountMonitoring.mockResolvedValue(mockJob);
            const response = await (0, supertest_1.default)(app)
            ...
            expect(HealthMonitorJob_1.queueAccountMonitoring).toHaveBeenCalledWith('acc123', 'user456');
        });
        it('should return 400 if userId is missing', async () => {
            ...
            expect(HealthMonitorJob_1.queueAccountMonitoring).not.toHaveBeenCalled();
        });
        it('should handle job queueing errors', async () => {
            HealthMonitorJob_1.queueAccountMonitoring.mockRejectedValue(new Error('Queue is full'));
            const response = await (0, supertest_1.default)(app)
                .post('/api/health/account/acc123/monitor')
                .send({ userId: 'user456' })
                .expect('Content-Type', /json/)
    ...
    describe('GET /api/health/swarm', () => {
        it('should return swarm health overview', async () => {
            ...
            HealthMonitor_1.default.monitorSwarm.mockResolvedValue(mockSwarmReport);
            const response = await (0, supertest_1.default)(app)
            ...
            expect(HealthMonitor_1.default.monitorSwarm).toHaveBeenCalledWith('user123');
        });
        it('should return 400 if userId is missing', async () => {
            const response = await (0, supertest_1.default)(app)
                .get('/api/health/swarm')
            ...
            expect(response.body.success).toBe(false);
            expect(response.body.error).toBe('User ID is required');
            expect(HealthMonitor_1.default.monitorSwarm).not.toHaveBeenCalled();
        });
        it('should handle monitoring errors', async () => {
            HealthMonitor_1.default.monitorSwarm.mockRejectedValue(new Error('Database connection failed'));
            const response = await (0, supertest_1.default)(app)
                .get('/api/health/swarm')
                .query({ userId: 'user123' })
                .expect('Content-Type', /json/)
        ...
        it('should handle empty swarm (no accounts)', async () => {
            ...
            HealthMonitor_1.default.monitorSwarm.mockResolvedValue(emptySwarmReport);
            const response = await (0, supertest_1.default)(app)
                .get('/api/health/swarm')
    ...
    describe('Error Handling', () => {
        it('should handle unexpected errors with 500 status', async () => {
            HealthMonitor_1.default.monitorAccount.mockRejectedValue(new Error('Unexpected error occurred'));
            const response = await (0, supertest_1.default)(app)
        ...
        it('should handle null/undefined return values', async () => {
            HealthMonitor_1.default.monitorAccount.mockResolvedValue(null);
            const response = await (0, supertest_1.default)(app)
                .get('/api/health/account/acc123')
                .expect(200);
            expect(response.body.success).toBe(true);
