...
const RateLimiter_1 = require("../../../services/instagram/RateLimiter");
...
describe('RateLimiter', () => {
    ...
    describe('checkLimit', () => {
        it('should check limits for different actions', async () => {
            ...
            const postResult = await rateLimiter.checkLimit('account123', errors_1.AccountAction.POST, 'personal');
            expect(postResult.allowed).toBe(true);
            const likeResult = await rateLimiter.checkLimit('account123', errors_1.AccountAction.LIKE, 'personal');
            expect(likeResult.allowed).toBe(true);
            const commentResult = await rateLimiter.checkLimit('account123', errors_1.AccountAction.COMMENT, 'personal');
        ...
        it('should apply reduced limits for new accounts', async () => {
            ...
            const result = await rateLimiter.checkLimit('newaccount', errors_1.AccountAction.POST, 'personal', twentyDaysAgo);
            expect(result.allowed).toBe(true);
        });
        it('should not reduce limits for old accounts', async () => {
            ...
            const result = await rateLimiter.checkLimit('oldaccount', errors_1.AccountAction.POST, 'personal', sixtyDaysAgo);
            expect(result.allowed).toBe(true);
        });
    });
    describe('recordAction', () => {
        it('should increment hourly and daily counters', async () => {
            ...
            await rateLimiter.recordAction('account123', errors_1.AccountAction.POST);
        ...
        it('should set correct TTL for hourly and daily windows', async () => {
            ...
            mockRedis.pipeline.mockReturnValue(pipeline);
            await rateLimiter.recordAction('account123', errors_1.AccountAction.LIKE);
            // Verify expire was called with correct TTLs
            const expireCalls = pipeline.expire.mock.calls;
    ...
    describe('getRemainingQuota', () => {
        it('should calculate remaining quota correctly', async () => {
            ...
            const result = await rateLimiter.getRemainingQuota('account123', errors_1.AccountAction.LIKE, 'personal');
        ...
        it('should handle no usage (fresh account)', async () => {
            ...
            const result = await rateLimiter.getRemainingQuota('newaccount', errors_1.AccountAction.POST, 'personal');
    ...
    describe('getResetTime', () => {
        it('should return TTL for both windows', async () => {
            ...
            const result = await rateLimiter.getResetTime('account123', errors_1.AccountAction.POST);
        ...
        it('should handle expired keys (TTL -2)', async () => {
            ...
            const result = await rateLimiter.getResetTime('account123', errors_1.AccountAction.COMMENT);
        ...
        it('should handle keys without TTL (TTL -1)', async () => {
            ...
            const result = await rateLimiter.getResetTime('account123', errors_1.AccountAction.FOLLOW);
            expect(result.hourlyResetIn).toBe(0);
            expect(result.dailyResetIn).toBe(0);
        });
    });
    describe('resetQuota', () => {
        it('should reset quota for specific action', async () => {
            ...
            await rateLimiter.resetQuota('account123', errors_1.AccountAction.POST);
            expect(mockRedis.del).toHaveBeenCalledWith(expect.stringContaining('account123'), expect.stringContaining('account123'));
        });
        it('should reset quota for all actions if no action specified', async () => {
            ...
            expect(mockRedis.del.mock.calls[0].length).toBe(10);
        });
    });
    describe('integration scenarios', () => {
        ...
        it('should correctly reduce limits for new accounts', async () => {
            ...
            const result = await rateLimiter.checkLimit('newaccount', errors_1.AccountAction.LIKE, 'personal', newAccount);
