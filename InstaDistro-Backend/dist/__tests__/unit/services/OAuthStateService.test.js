...
describe('OAuthStateService', () => {
    let service;
    beforeEach(() => {
        service = new OAuthStateService_1.OAuthStateService();
        jest.clearAllMocks();
    });
    ...
    describe('storeState', () => {
        it('should store state in Redis with metadata', async () => {
            ...
            redis_1.redisClient.setex.mockResolvedValue('OK');
            await service.storeState(state, metadata);
            expect(redis_1.redisClient.setex).toHaveBeenCalledWith('oauth:state:test-state-token', 600, // TTL
            JSON.stringify(metadata));
        });
        it('should throw error if Redis fails', async () => {
            const state = 'test-state';
            const metadata = { provider: 'google', timestamp: Date.now() };
            redis_1.redisClient.setex.mockRejectedValue(new Error('Redis error'));
            await expect(service.storeState(state, metadata)).rejects.toThrow('Failed to store OAuth state');
        });
    });
    describe('validateState', () => {
        it('should return metadata for valid state', async () => {
            ...
            redis_1.redisClient.get.mockResolvedValue(JSON.stringify(metadata));
            redis_1.redisClient.del.mockResolvedValue(1);
            const result = await service.validateState(state);
            expect(result).toEqual(metadata);
            expect(redis_1.redisClient.get).toHaveBeenCalledWith('oauth:state:valid-state');
            expect(redis_1.redisClient.del).toHaveBeenCalledWith('oauth:state:valid-state');
        });
        it('should return null for non-existent state', async () => {
            redis_1.redisClient.get.mockResolvedValue(null);
            const result = await service.validateState('invalid-state');
            expect(result).toBeNull();
            expect(redis_1.redisClient.del).not.toHaveBeenCalled(); // Should not delete if not found
        });
        ...
        it('should delete state after successful validation (one-time use)', async () => {
            ...
            expect(redis_1.redisClient.del).toHaveBeenCalledWith('oauth:state:one-time-state');
        });
        it('should return null if Redis fails', async () => {
            redis_1.redisClient.get.mockRejectedValue(new Error('Redis error'));
            const result = await service.validateState('test-state');
            expect(result).toBeNull();
        });
        it('should return null for invalid JSON in Redis', async () => {
            redis_1.redisClient.get.mockResolvedValue('invalid-json{');
            const result = await service.validateState('test-state');
            expect(result).toBeNull();
        });
    });
    describe('cleanupExpiredStates', () => {
        it('should delete states without TTL', async () => {
            ...
            redis_1.redisClient.keys.mockResolvedValue(keys);
            redis_1.redisClient.ttl
            ...
            expect(redis_1.redisClient.del).toHaveBeenCalledTimes(2);
            expect(redis_1.redisClient.del).toHaveBeenCalledWith('oauth:state:key1');
            expect(redis_1.redisClient.del).toHaveBeenCalledWith('oauth:state:key3');
        });
        it('should return 0 if no states need cleanup', async () => {
            ...
            expect(redis_1.redisClient.del).not.toHaveBeenCalled();
        });
        it('should return 0 if no states exist', async () => {
            redis_1.redisClient.keys.mockResolvedValue([]);
            const result = await service.cleanupExpiredStates();
            expect(result).toBe(0);
        });
        it('should return 0 if Redis fails', async () => {
            redis_1.redisClient.keys.mockRejectedValue(new Error('Redis error'));
            const result = await service.cleanupExpiredStates();
            expect(result).toBe(0);
        });
    });
    describe('integration scenarios', () => {
        it('should handle complete OAuth flow', async () => {
            ...
            redis_1.redisClient.setex.mockResolvedValue('OK');
            await service.storeState(state, metadata);
            expect(redis_1.redisClient.setex).toHaveBeenCalled();
            // Validate state
            redis_1.redisClient.get.mockResolvedValue(JSON.stringify(metadata));
            redis_1.redisClient.del.mockResolvedValue(1);
            const result = await service.validateState(state);
